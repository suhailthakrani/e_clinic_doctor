...
/// Created by Amjad Jamali 7-Aug-2023
class LoginScreenController extends GetxController {

  GlobalKey<ScaffoldState> scaffoldKey = GlobalKey<ScaffoldState>();

  TextFieldManager usernameManager = TextFieldManager('Username', length: 50,filter: TextFilter.email);
  TextFieldManager passwordManager = TextFieldManager('Password', length: 50,filter: TextFilter.none);
  ...
  void onLoginClick() async{
    if(usernameManager.validate() & passwordManager.validate()){
      ...
      UserModel userModel = await UserService().loginUser(username: usernameManager.text, password: passwordManager.text);
      pd.dismissDialog();
