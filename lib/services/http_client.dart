...
class HTTPClient extends GetConnect{
  ...
  Future<ResponseModel> postRequest({required String url, dynamic requestBody, bool requireToken = true}) async {
    if(!(await CommonCode().checkInternetConnection())){
      return Future.value(ResponseModel.named(message:kNoInternetMsg, data:kNoInternetMsg));
    }
    ...
    try {
      ...
      return Future.value(ResponseModel.named(message: "Request TimeOut", data: kPoorInternetConnection));
    } on SocketException {
      return Future.value(ResponseModel.named(message: "Bad Request", data: kNetworkError));
    } catch (e) {
      if(!(await CommonCode().checkInternetAccess())){
        return Future.value(ResponseModel.named(message:kPoorInternetConnection, data:kPoorInternetConnection));
      }
      return Future.value(ResponseModel.named(message:kNetworkError, data: kNetworkError));
    }
  }
  ...
  Future<ResponseModel> postMultipartRequest({required String url, Map<String, String> fields =const{}, Map<String, String> files =const{}}) async {
    ...
    try {
      ...
      return Future.value(ResponseModel.named(message: "Request TimeOut", data: "Request TimeOut"));
    } on SocketException {
      return Future.value(ResponseModel.named(message: "Bad Request", data: "Bad Request"));
    } catch (e) {
      if(!(await CommonCode().checkInternetAccess())){
        return Future.value(ResponseModel.named(message:kPoorInternetConnection, data:kPoorInternetConnection));
      }
